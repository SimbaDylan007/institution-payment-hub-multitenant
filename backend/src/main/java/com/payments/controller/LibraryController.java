...
@PreAuthorize("hasAnyRole('ADMIN', 'ADMINISTRATOR', 'LIBRARIAN', 'SUPER_ADMIN')")
public class LibraryController {
    ...
    public ResponseEntity<Page<Book>> getAllBooks(
        ...
        return ResponseEntity.ok(libraryService.getAllBooks(searchTerm, pageable, institutionId));
    }

    @PostMapping("/books/bulk-upload")
    public ResponseEntity<?> bulkAddBooks(
            @RequestParam("file") MultipartFile file,
            @RequestParam(required = false) Long institutionId) { // <-- Accept optional institutionId
        try {
            // Pass institutionId to the service method
            return ResponseEntity.ok(libraryService.bulkAddBooks(file, institutionId));
    ...
    public ResponseEntity<Book> updateBook(@PathVariable Long id, @RequestBody Book bookDetails) {
        return ResponseEntity.ok(libraryService.updateBook(id, bookDetails));
    }
    ...
    public ResponseEntity<?> returnBook(@RequestParam Long bookId, @RequestParam String studentId) {
        try {
            return ResponseEntity.ok(libraryService.returnBook(bookId, studentId));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(e.getMessage());
        }
    }
    ...
    public ResponseEntity<Page<BookTransaction>> getLoanHistory(
            ...
            Pageable pageable) {
        // Pass institutionId to the service method
        return ResponseEntity.ok(libraryService.getLoanHistory(studentId, pageable, institutionId));
    }
}