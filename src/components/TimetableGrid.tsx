...
export default function TimetableGrid({ grade, section, onEditEntry, onAddEntry }: TimetableGridProps) {
  ...
  const fetchTimetable = async () => {
    ...
    try {
      ...
      const response = await apiFetch(`http://localhost:8080/api/timetables/grade/${selectedGrade}/section/${selectedSection}`);
  ...
  const deleteEntry = async (id: number) => {
    try {
      ...
      const response = await apiFetch(`http://localhost:8080/api/timetables/${id}`, {
        method: 'DELETE'
      });
      if (response.ok) {
