...
import { useAuth } from "@/contexts/AuthContext"; // 1. IMPORT useAuth TO GET THE USER'S ROLE
...
// 2. DEFINE the permissions for each navigation item.
...
const rolePermissions: { [key: string]: string[] } = {
  ...
  "Settings": ["ADMIN", "IT_ADMIN"], // Only Admin and IT_Admin can see settings
...
export default function SchoolNavigation() {
  ...
  const { user } = useAuth(); // 3. GET the currently logged-in user from the context

  // 4. FILTER the full list of navigation items down to only what the current user is allowed to see.
  ...
  return (
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {/* 5. RENDER the filtered list of accessible items, not the original full list. */}
