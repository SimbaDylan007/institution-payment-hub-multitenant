  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setOpen(false);
        onAttendanceMarked?.(); // Callback to refresh parent component data
      } else {
        // Try to get a more specific error message from the server response
        const errorData = await response.json().catch(() => ({ message: "Failed to mark attendance. Please try again." }));
