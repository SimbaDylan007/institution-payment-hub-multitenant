  ...
  return (
      <div className="min-h-screen bg-gradient-to-br from-black via-red-900 to-white-900 text-white flex flex-col">
        ...
        <main className="flex-1 px-8 py-8">
          <div className="mb-6 flex justify-between items-center"><h1 className="text-2xl font-bold">Academic Management</h1><Button asChild className="bg-red-600"><Link to="/dashboard" className="flex items-center gap-2"><Home className="h-4 w-4 mr-2"/>Dashboard</Link></Button></div>
          <Tabs defaultValue="subjects" className="space-y-6">
            ...
            <TabsContent value="subjects">
              <Card className="bg-gradient-to-br from-red-900/50 to-white-900/50 border-red-700">
                <CardHeader>
                  <div className="flex justify-between items-center"><CardTitle className="flex items-center gap-2"><BookOpen/>Subject Management</CardTitle><div className="flex gap-2"><Button onClick={() => setIsSubjectImportOpen(true)} className="bg-white-600 hover:bg-white-700"><UploadCloud className="h-4 w-4 mr-2"/>Import Subjects</Button><Button onClick={() => { setSelectedSubject(null); setIsSubjectDialogOpen(true); }} className="bg-green-600 hover:bg-green-700"><Plus className="h-4 w-4 mr-2"/>Add Subject</Button></div></div>
                  <div className="flex gap-4 pt-4"><Input placeholder="Search by name or code..." value={subjectSearch} onChange={e => {setSubjectSearch(e.target.value); setSubjectPageNum(0);}} className="bg-red-800 border-red-600" /><Select value={subjectGradeFilter} onValueChange={v => {setSubjectGradeFilter(v); setSubjectPageNum(0);}}><SelectTrigger className="w-[180px] bg-red-800 border-red-600"><SelectValue/></SelectTrigger><SelectContent className="bg-red-800"><SelectItem value="All">All Grades</SelectItem>{[...Array(12)].map((_,i) => <SelectItem key={i} value={`${i+1}`}>{`Grade ${i+1}`}</SelectItem>)}</SelectContent></Select></div>
                </CardHeader>
                <CardContent><div className="space-y-4">{loading && !subjectPage?.content ? <p className="text-center p-4">Loading subjects...</p> : subjectPage?.content.map((subject) => (<div key={subject.id} className="flex justify-between items-center p-4 bg-red-800/30 rounded-lg border border-red-600"><div><h3 className="font-semibold">{subject.name} ({subject.code})</h3><p className="text-sm text-gray-300">Grade {subject.grade} • {subject.credits} Credits</p></div><div className="flex gap-2"><Button size="sm" variant="outline" className="hover:bg-red-700" onClick={() => { setSelectedSubject(subject); setIsSubjectDialogOpen(true); }}><Edit className="h-4 w-4" /></Button><Button size="sm" variant="outline" className="text-red-400 hover:bg-red-600 hover:text-white" onClick={() => handleDeleteSubject(subject.id)}><Trash2 className="h-4 w-4" /></Button></div></div>))}</div><PaginationControls page={subjectPage} onPageChange={setSubjectPageNum} /></CardContent>
              </Card>
            </TabsContent>
            ...
            <TabsContent value="grades">
              <Card className="bg-gradient-to-br from-red-900/50 to-white-900/50 border-red-700">
                <CardHeader>
                  <div className="flex justify-between items-center"><CardTitle className="flex items-center gap-2"><GraduationCap/>Grade Management</CardTitle><div className="flex gap-2"><Button onClick={() => setIsGradeImportOpen(true)} className="bg-white-600 hover:bg-white-700"><UploadCloud className="h-4 w-4 mr-2"/>Import Grades</Button><Button onClick={() => { setSelectedGrade(null); setIsGradeDialogOpen(true); }} className="bg-green-600 hover:bg-green-700"><Plus className="h-4 w-4 mr-2"/>Add Grade</Button></div></div>
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4 pt-4">
                    <Input placeholder="Search Student ID or Subject Code..." value={gradeSearch} onChange={e => {setGradeSearch(e.target.value); setGradePageNum(0);}} className="bg-red-800 border-red-600 md:col-span-2" />
                    <Select value={gradeYearFilter} onValueChange={v => {setGradeYearFilter(v); setGradePageNum(0);}}><SelectTrigger className="bg-red-800"><SelectValue placeholder="All Years"/></SelectTrigger><SelectContent className="bg-red-800"><SelectItem value="All">All Years</SelectItem><SelectItem value="2024-2025">2024-2025</SelectItem><SelectItem value="2023-2024">2023-2024</SelectItem></SelectContent></Select>
                    <Select value={gradeSemesterFilter} onValueChange={v => {setGradeSemesterFilter(v); setGradePageNum(0);}}><SelectTrigger className="bg-red-800"><SelectValue placeholder="All Semesters"/></SelectTrigger><SelectContent className="bg-red-800"><SelectItem value="All">All Semesters</SelectItem><SelectItem value="SEMESTER_1">Semester 1</SelectItem><SelectItem value="SEMESTER_2">Semester 2</SelectItem></SelectContent></Select>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="overflow-x-auto"><table className="w-full text-left"><thead><tr className="border-b border-red-600"><th className="p-2">Student ID</th><th className="p-2">Subject</th><th className="p-2">Marks</th><th className="p-2">Grade</th><th className="p-2 text-center">Actions</th></tr></thead><tbody>
                  {loading && !gradePage?.content ? (<tr><td colSpan={5} className="text-center p-4">Loading grades...</td></tr>) :
                      gradePage?.content.map(grade => (<tr key={grade.id} className="border-b border-red-800">
        ...
        <Dialog open={isSubjectDialogOpen} onOpenChange={(isOpen) => { if (!isOpen) setSelectedSubject(null); setIsSubjectDialogOpen(isOpen); }}><DialogContent className="bg-red-900 border-red-700 text-white"><DialogHeader><DialogTitle>{selectedSubject ? 'Edit Subject' : 'Add New Subject'}</DialogTitle></DialogHeader><form onSubmit={handleSubjectSubmit} className="space-y-4"><div><Label htmlFor="name">Subject Name</Label><Input id="name" name="name" defaultValue={selectedSubject?.name} required className="bg-red-800"/></div><div><Label htmlFor="code">Subject Code</Label><Input id="code" name="code" defaultValue={selectedSubject?.code} required className="bg-red-800"/></div><div className="grid grid-cols-2 gap-4"><div><Label htmlFor="grade">Grade Level</Label><Select name="grade" defaultValue={selectedSubject?.grade}><SelectTrigger className="bg-red-800"><SelectValue placeholder="Select..."/></SelectTrigger><SelectContent className="bg-red-800">{[...Array(12)].map((_,i) => <SelectItem key={i} value={`${i+1}`}>{`Grade ${i+1}`}</SelectItem>)}</SelectContent></Select></div><div><Label htmlFor="credits">Credits</Label><Input id="credits" name="credits" type="number" defaultValue={selectedSubject?.credits} required className="bg-red-800"/></div></div><div><Label htmlFor="description">Description</Label><Textarea id="description" name="description" defaultValue={selectedSubject?.description} className="bg-red-800"/></div><div className="flex justify-end gap-2"><Button type="button" variant="outline" onClick={() => setIsSubjectDialogOpen(false)}>Cancel</Button><Button type="submit" disabled={loading}>{loading ? 'Saving...' : 'Save'}</Button></div></form></DialogContent></Dialog>
        <Dialog open={isGradeDialogOpen} onOpenChange={(isOpen) => { if (!isOpen) setSelectedGrade(null); setIsGradeDialogOpen(isOpen); }}><DialogContent className="bg-red-900 border-red-700 text-white"><DialogHeader><DialogTitle>{selectedGrade ? 'Edit Grade' : 'Add New Grade'}</DialogTitle></DialogHeader><form onSubmit={handleGradeSubmit} className="space-y-4"><div><Label htmlFor="studentId">Student ID</Label><Input id="studentId" name="studentId" defaultValue={selectedGrade?.student?.studentId} required className="bg-red-800"/></div><div><Label htmlFor="subjectId">Subject Code</Label><Input id="subjectId" name="subjectId" defaultValue={selectedGrade?.subject?.code} required className="bg-red-800"/></div><div className="grid grid-cols-2 gap-4"><div><Label htmlFor="assessmentType">Assessment Type</Label><Select name="assessmentType" defaultValue={selectedGrade?.assessmentType}><SelectTrigger className="bg-red-800"><SelectValue/></SelectTrigger><SelectContent className="bg-red-800"><SelectItem value="MIDTERM">Midterm</SelectItem><SelectItem value="FINAL">Final</SelectItem></SelectContent></Select></div><div><Label htmlFor="letterGrade">Letter Grade</Label><Input id="letterGrade" name="letterGrade" defaultValue={selectedGrade?.letterGrade} required className="bg-red-800"/></div></div><div className="grid grid-cols-2 gap-4"><div><Label htmlFor="marksObtained">Marks Obtained</Label><Input id="marksObtained" name="marksObtained" type="number" defaultValue={selectedGrade?.marksObtained} required className="bg-red-800"/></div><div><Label htmlFor="maxMarks">Max Marks</Label><Input id="maxMarks" name="maxMarks" type="number" defaultValue={selectedGrade?.maxMarks} required className="bg-red-800"/></div></div><div className="grid grid-cols-2 gap-4"><div><Label htmlFor="academicYear">Academic Year</Label><Input id="academicYear" name="academicYear" defaultValue={selectedGrade?.academicYear} required className="bg-red-800"/></div><div><Label htmlFor="semester">Semester</Label><Select name="semester" defaultValue={selectedGrade?.semester}><SelectTrigger className="bg-red-800"><SelectValue/></SelectTrigger><SelectContent className="bg-red-800"><SelectItem value="SEMESTER_1">Semester 1</SelectItem><SelectItem value="SEMESTER_2">Semester 2</SelectItem></SelectContent></Select></div></div><div className="flex justify-end gap-2"><Button type="button" variant="outline" onClick={() => setIsGradeDialogOpen(false)}>Cancel</Button><Button type="submit" disabled={loading}>{loading ? 'Saving...' : 'Save'}</Button></div></form></DialogContent></Dialog>
      </div>
  );
}