...
import { toast } from "sonner"; // Although not used directly, good to have

const Auth = () => {
  const { user, login, isLoading } = useAuth();

  const [username, setUsername] = useState("smach"); // Pre-filled for easy testing
  const [password, setPassword] = useState("simba"); // Pre-filled for easy testing
  const [showPassword, setShowPassword] = useState(false);
  const [rememberMe, setRememberMe] = useState(true);

  // The error message state is no longer needed as AuthContext handles it.

  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();

    if (!username || !password) {
      // Use toast for client-side validation
      toast.error("Please fill in all fields.");
      return;
    }

    // The login function in AuthContext will now handle all logic,
    // including showing success or error toasts.
        ...
        <div className="hidden md:block md:w-1/2 bg-gradient-to-br from-purple-600 to-purple-800 p-12 flex flex-col justify-center">
          <div className="max-w-lg">
            ...
            <ul className="space-y-4">
              ...
              <li className="flex items-center"><div className="mr-4 text-xl">•</div><div>Comprehensive reporting and analytics</div></li>
              <li className="flex items-center"><div className="mr-4 text-xl">•</div><div>Secure and reliable payment data management</div></li>
            </ul>
          </div>
        </div>
      </div>
