...
export default function Schedule() {
  ...
  const fetchEvents = useCallback(async () => {
    ...
    try {
      // 2. Use apiFetch
      const response = await apiFetch(`/api/schedule/events?year=${year}&month=${month}`);
      if (response.ok) {
  ...
  const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await apiFetch('/api/schedule/events', { method: 'POST', body: JSON.stringify(apiEvent) });
      if (response.ok) {
  ...
  const handleDeleteEvent = async () => {
    ...
    setLoading(true);
    try {
      const response = await apiFetch(`/api/schedule/events/${selectedEvent.id}`, { method: 'DELETE' });
      if (response.ok) {
  ...
  const handleImport = async () => {
    ...
    try {
      const response = await apiFetch('/api/schedule/timetables/bulk-upload', { method: 'POST', body: formData });
      if (response.ok) {
